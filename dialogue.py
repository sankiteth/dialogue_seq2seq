	...
	with gfile.GFile(data_path, mode="r") as fi:
		for line in fi.readlines():
			...
			if len(entities) == 3:
				...
				source_1_ids = [int(x) for x in sentence_to_token_ids(source_1,vocab)]
				target_1_ids = [int(x) for x in sentence_to_token_ids(target_1,vocab)]
				target_1_ids.append(EOS_ID)

				source_2_ids = [int(x) for x in sentence_to_token_ids(source_2,vocab)]
				target_2_ids = [int(x) for x in sentence_to_token_ids(target_2,vocab)]
				target_2_ids.append(EOS_ID)
...
def train():
  """Train a dialogue system using 3 turn data"""
  print("In train")
  ...
  with tf.Session() as sess:
    ...
    while True:
      ...
      if current_step % FLAGS.steps_per_checkpoint == 0:
        ...
        perplexity = math.pow(2, float(loss)) if loss < 300 else float("inf")
        ...
        for bucket_id in xrange(len(_buckets)):
          ...
          eval_ppx = math.pow(2, float(eval_loss)) if eval_loss < 300 else float(
              "inf")
